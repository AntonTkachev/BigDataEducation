...
>  Проблема маленьких файлов, подробнее!!!  
...
это общие переменные только для чтения, которые кэшируются и доступны на всех узлах кластера для доступа к задачам или их использования. Вместо того, чтобы отправлять эти данные вместе с каждой задачей, spark распределяет широковещательные переменные на машину,
если воркер изменит эту переменную, она будет доступна только ему
...
**Schema enforcement** — является защитой в Delta Lake, которая обеспечивает качество данных, отклоняя записи в таблицу, которые не соответствуют схеме таблицы.
...
- Sink – дпредназначен для выгрузки данных из Kafka
...
[//]: # (nosql какие типы есть)

[//]: # (ELK stack - BEATS, что именно нужно)
[//]: # (VM/Container)

[//]: # (airflow, сенсор - разобрать. Executor)
[//]: # (NIFI)
[//]: # (Spark/ core/ )
