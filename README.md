...
- **Secondary NameNode** — отвечает за объединение editlogs с fsimage из namenode.
  - Получает editlogs из NameNode и через регулярные интервалы, применяет к fsimage
  - Как только у него появляется новый fsimage, он копирует его обратно в NameNode
  - Namenode будет использовать этот fsimage для следующего перезапуска, что сократит время запуска
...
Методы разделяющие DataFrame на части, repartition на примерно одинаковые, так же может уменьшать или увеличивать число разделов  
сoalesce - только уменьшать, быстрее без Shuffle???
